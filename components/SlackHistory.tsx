      ...
      <div className="flex-1 overflow-y-auto space-y-2">
        ...
        {!connected && (
          <div className="flex flex-col items-center gap-3 py-8 px-2">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 text-center">Connect Slack to see your message history.</p>
            <button
        ...
        {posts.map((post) => (
          <div
            key={post.id}
            className="bg-gray-50 dark:bg-gray-800/60 rounded-lg p-3 border border-gray-200 dark:border-gray-700/50 hover:border-gray-300 dark:hover:border-gray-600 transition-colors"
          >
            <div className="flex items-start justify-between gap-2">
              <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 truncate">{post.title}</p>
              <div className="text-right flex-shrink-0">
                <p className="text-xs text-gray-500">{formatDate(post.posted_at)}</p>
                <p className="text-xs text-gray-400 dark:text-gray-600">{formatTime(post.posted_at)}</p>
              </div>
            </div>
            {post.body && (
