...
function StatusBadge({ connected }: { connected: boolean }) {
  return (
    ...
    >
      <span className={`w-1.5 h-1.5 rounded-full ${connected ? "bg-green-500 dark:bg-green-400" : "bg-gray-400 dark:bg-gray-500"}`} />
  ...
  return (
    <div className="min-h-screen p-8 max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <a href="/" className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-400 text-sm">
          &larr; Dashboard
        </a>
        <ThemeToggle />
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" className="text-gray-800 dark:text-white">
                ...
                <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Notion</h3>
              </div>
          ...
          <button
            ...
            className="mt-4 w-full bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm font-medium py-2.5 rounded-xl transition-colors border border-gray-200 dark:border-gray-700"
          >
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none focus:border-blue-500 transition-colors font-mono"
                />
                <button
                  onClick={saveDbId}
                  disabled={dbSaving || !dbId.trim()}
                  className="px-4 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-200 dark:disabled:bg-gray-700 disabled:text-gray-400 dark:disabled:text-gray-500 text-white text-sm font-medium rounded-xl transition-colors"
                >
                  {dbSaving ? "Saving…" : dbSaved ? "Saved ✓" : "Save"}
                </button>
              </div>
              {dbError && <p className="text-xs text-red-500 mt-1.5">{dbError}</p>}
            </div>
          )}
        </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Slack</h3>
              </div>
      ...
      {(notionConnected || slackConnected) && (
        <div className="mt-4 bg-green-500/10 border border-green-500/20 rounded-xl p-4 text-sm text-green-600 dark:text-green-400">
